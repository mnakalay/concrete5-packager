...
// doing dataObj.finalIgnoreList = ignore make both ignore and finalignoreList reference the same object
// as a result modifying finalIgnoreList ends up modifying ignore as WEBGL_lose_context.
// By doing slice we create a shallow copy of ignore
// dataObj.finalIgnoreList = ignore.slice(0)
...
function createData() {
    return {
        ...
        deleteBuildFolder: true,
        addC5Exec: true,
        finalIgnoreList: ignore.slice(0),
        filesToClean: [],
        reset: function() {
...
async function setDefaults(rootDir, pkgHandle) {
    ...
    var extensionExclusions = settings.getAppSettings('extensionExclusions', '')
    if (extensionExclusions.length) {
        extensionExclusions = extensionExclusions.split(',')
            // removing all the extra spaces if any and concatenating
        extensionExclusions = '*(**/)*.{' + extensionExclusions.map(str => str.trim()).join(',') + '}'
        dataObj.finalIgnoreList.push(extensionExclusions)
    }

    var fileFolderExclusions = settings.getAppSettings('fileFolderExclusions', '')
    if (fileFolderExclusions.length) {
        fileFolderExclusions = fileFolderExclusions.split(',')
            // removing all the extra spaces if any and concatenating
        fileFolderExclusions = '*(**/)*{' + fileFolderExclusions.map(str => str.trim()).join(',') + '}'
        dataObj.finalIgnoreList.push(fileFolderExclusions)
    }
}

async function setCustomExclusions() {
    const exclusionsFile = dataObj.srcDir + path.sep + 'exclusions.json'
    if (!checkFileExists(exclusionsFile)) {
        return
    }

    try {
        const data = await fs.readJson(exclusionsFile, 'utf-8')
        ...
        if (filesfolders) {
            ...
            dataObj.finalIgnoreList.push(filesfolders)
        }
        ...
        if (extensions) {
            ...
            dataObj.finalIgnoreList.push(extensions)
        }
...
async function zipIt() {
    ...
    output.on('close', function(e) {
        ...
        global.mainWindow.webContents.send(
            'zipped', {
                success: true,
                msg: 'Your package archive ' + dataObj.pkgHandle + '.zip (' + formatBytes(archive.pointer(), 2) + ') was created successfully in your release folder',
